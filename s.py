...
@torch.no_grad()
def sample(generator, step, mean_style, vector, device):
    image = generator(
        torch.from_numpy(vector).float().to(device),
        step=step,
        alpha=1,
        mean_style=mean_style,
        style_weight=0.7,
    )
    #print(torch.randn((1,512)))
    return image
...
if __name__ == '__main__':
    ...
    parser.add_argument('--latent', type=str, help='latent')
    ...
    vector=np.load('res.npy')
    img = sample(generator, step, mean_style, vector, device)
